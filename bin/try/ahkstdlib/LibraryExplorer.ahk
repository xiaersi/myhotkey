...
showGui(buildMainGui(), 1)                              ; Create and show the Gui.
EmptyMem()
SetBatchLines, 0
...
buildMainGui()
{
    ...
    Local standardFonts := "Verdana, Segoe UI"
    Local sourceFonts := "Courier New, Lucida Console"
    ...
    Gui, Add, Text, vTxt_CurrentAhkVersion 0x80
        , % "Your installed AutoHotkey version is " . ini_getValue(config, "Version", "currentAhk") . "."
    setGuiFont("", "cblue underline")
    Gui, Add, Text, x+4 +0x400 0x80 gTxt_openHome vTxt_openHome, Home of Collection
    ...
    w := 40
    Gui, Add, Text, w%w%, Search:
    w := Base_w - w - SmallButton_w - (mx2 * 15) ; mx2 * x for all following controls.
    Gui, Add, Checkbox, w%SmallButton_w% x+%mx% +Check3 +Checked-1 gChk_isStandalone vChk_isStandalone, Standalone 
    Gui, Add, Checkbox, w%SmallButton_w% x+%mx% +Check3 +Checked-1 gChk_isStdlibConform vChk_isStdlibConform, StdlibConform 
    ...
    Gui, Add, Text, w%w% x%x% yp-44 0x80 +Right +Hidden vTxt_Warning
    setGuiFont("default")
    setGuiFont("", "norm")
    ...
    Gui, Add, Button, w%w% gBtn_openDoc vBtn_openDoc, Open Help
...
getGuiTitle(_updateCount = "")
{
    ...
    GuiTitle := ini_getValue(database_header, "HEADER", "Title")
        . ", " . date . " (by Tuncay) ~ Libs: " . LV_GetCount() . " / " . currentCount
    Return GuiTitle
}
...
Ed_Search(_forceSearch = False)
{
    ...
    If (Ed_Search != "" || _forceSearch = True)
    {
        ...
        Loop, % LV_GetCount()
        {
            ...
            Haystack = %Haystack%
;~             If (   (searchType > 0 && searchType < 4 && (Haystack = Ed_Search))
;~                 || (searchType = 4 && InStr(Haystack, Ed_Search))
;~                 || (RegExMatch(Haystack, "Sim)" . Ed_Search)) )
;~             {
;~                 Gui, ListView, Lv_Database2
...
(
Search is done via Regular Expression, the RegExMatch() function (options cannot be specified). It sees every word as an individual search term, separated by space. All of these terms must match to show the entry (or with prefixed "!" to NOT match).
...
Txt_openHome:
Run, % ini_getValue(database_header, "HEADER", "Location")
Return
...
Btn_runExample()
{
    ...
    FileAppend, %Ed_InfoExample%`r`nRETURN`r`n#Include %libpath%%DependFiles%`r`n#Include %source%, %sample%
...
HandleMessage( p_w, p_l, p_m, p_hw )
{
   ...
   else if ( p_m = WM_MOUSEMOVE )
   {
      if ( A_GuiControl = "Txt_openHome" )
      {
         if URL_hover=
         {
            setGuiFont("Verdana, Segoe UI", "norm cFF7F00")
      ...
      else
      {
         if ( URL_hover )
         {
            setGuiFont("Verdana, Segoe UI", "underline cBlue")
            GuiControl, Font, Txt_openHome
            setGuiFont("default", "norm")
            setGuiFont("")
