...
ifexist %inifile%	; 从配置文件中读取默认值
{
	iniread var_temp, %inifile%, install, smtp
	if var_temp <> and var_temp <> ERROR
	{
		GuiControl ,, Server, %var_temp%
	}
	iniread var_temp, %inifile%, install, mail
	if var_temp <> and var_temp <> ERROR
	{
		GuiControl ,, MailAddr, %var_temp%
	}
	iniread var_temp, %inifile%, install, repeat
	if var_temp <> and var_temp <> ERROR
	{
		GuiControl ,, Repeat, %var_temp%
	}
	iniread var_temp, %inifile%, install, port
	if var_temp <> and var_temp <> ERROR
	{
		GuiControl ,, Port, %var_temp%
	}
}
else
{
	iniwrite %Server%, %inifile%, install, smtp
	iniwrite %MailAddr%, %inifile%, install, mail
	iniwrite %Repeat%, %inifile%, install, repeat
	iniwrite %Port%, %inifile%, install, port
}
...
OkButton:
	...
	runwait,%comspec% /c blat -install %Server% %MailAddr% %Repeat% %Port%, %A_scriptdir%,Hido
	msgbox 邮箱信息安装完毕！
	ExitAPP
	return
