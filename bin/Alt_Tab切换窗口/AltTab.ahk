...
NOTE: Stroke-It (and maybe other mouse gesture programs) can cause the context menu to be shown twice/problematic.
        Solution = exclude the program within the gesture recognition program (window title = %g_title%).
...
GoSub Alt_Tab                           ;; add by teshorse, Show ListView while run this script.
...
Initiate_Hotkeys:
  ...
  Hotkey, %Alt_Hotkey%``, Alt_Esc
  Hotkey, %Alt_Hotkey%1,    【快捷切换到指定序号的窗口】
  Hotkey, %Alt_Hotkey%2,    【快捷切换到指定序号的窗口】
  Hotkey, %Alt_Hotkey%3,    【快捷切换到指定序号的窗口】
  Hotkey, %Alt_Hotkey%4,    【快捷切换到指定序号的窗口】
  Hotkey, %Alt_Hotkey%6,    【快捷切换到指定序号的窗口】
  Hotkey, %Alt_Hotkey%7,    【快捷切换到指定序号的窗口】
  Hotkey, %Alt_Hotkey%8,    【快捷切换到指定序号的窗口】
  Hotkey, %Alt_Hotkey%9,    【快捷切换到指定序号的窗口】
...
Alt_Tab_Common_Stuff:
  ...
  If Display_List_Shown =0
    ...
    {
		Gui, 1: Show, AutoSize x%Gui_x_Now% y%Gui_y_Now%, %g_title%
		settimer 【ListView中移动鼠标时选中鼠标所在行】, 60
		;LVA_ListViewAdd("ListView1", "AR AC cbsilver")
	}
...
Display_List:
  ...
  If Display_List_Shown !=1 ; no need to create gui for updating listview
	...
	Gui, 1: Add, ListView, x-1 y-1 r%Window_Found_Count% w%Listview_Width% AltSubmit -Multi -Hdr NoSort Count10 gListView_Event vListView1,#| |Window|Exe
    }
...
*/
 ...
 ; LVA_ListViewAdd("ListView1")
 /*
  LVA_ListViewAdd("ListView1", "AR AC cbsilver")
  LVA_SetCell("ListView1", 0, 1, "", "Green" )
  LVA_SetCell("ListView1", 0, 2, "", "0x3366BB" )
  LVA_SetCell("ListView1", 0, 3, "", "0x666666" )
*/
...
ListView_Destroy:
  ...
  settimer, 【ListView中移动鼠标时选中鼠标所在行】, off
  sleep 100
  ExitApp                             ;; add by teshorse, exitApp when listView destroyed
Return
...
【快捷切换到指定序号的窗口】:
  if RegExMatch( A_ThisHotkey, "\d+", Selected_Row ) > 0
  {
    Gosub, ListView_Get_Selected_and_Ensure_Visible
  }
  Return

【ListView中移动鼠标时选中鼠标所在行】:
	IfWinActive %g_title%
	{
		if IsMouseMove()
		{
			GetLVCellUnderMouse( Gui_ID, "SysListView321", _CurRow, _CurCol, _CellX, _CellY, _CellW, _CellH )
			Selected_Row := _CurRow
			Gosub, ListView_Get_Selected_and_Ensure_Visible
		}
	}
	Return
...
{
	global
    MouseGetPos,var_x,var_y,var_winid, var_ctrl
	WinGetTitle, var_title, ahk_id %var_winid%
	if ( var_title == g_title )
	{
		if g_oldMousePos_y =
		{
			g_oldMousePos_y := var_y
		}
		else if (  abs( var_y - g_oldMousePos_y ) > 20 )
		{
			g_oldMousePos_y := var_y
			return true
		}
